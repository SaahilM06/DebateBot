...
const App = () => {
  ...
  const loadConversation = async (id: string) => {
    ...
    if (!convoData.error) {
      ...
      const liveRes = await fetch("http://localhost:8000/live-read/");
      const liveData = await liveRes.json();
      setTranscript(liveData.transcript || "");
    }
  };
  ...
  const handleVectorize = async () => {
    ...
    const res = await fetch(`http://localhost:8000/run-vectorize/${conversationId}`, { method: "POST" });
    const result = await res.json();
    if (result.message) {
      alert(result.message);
      setHasStarted(true);
  ...
  {transcript && (
    <div className="self-end">
      <SpeechBubble text={transcript} isUser={true} />
    </div>
  )}
  {gptResponse && (
    <div className="self-start">
      <SpeechBubble text={gptResponse} isUser={false} />
    </div>
  )}
</div>
